...
def print_clustering_metrics(true_labels, predicted_labels):
    ...
    num_labels = len(np.unique(predicted_labels))
    n_samples = len(predicted_labels)

    if 2 <= num_labels < n_samples:
        ari = adjusted_rand_score(true_labels, predicted_labels)
        ami = adjusted_mutual_info_score(true_labels, predicted_labels)
        silhouette_avg = silhouette_score(data_normalized, predicted_labels)
        davies_bouldin = davies_bouldin_score(data_normalized, predicted_labels)
        ...
        print("Silhouette Score: Not applicable (invalid number of clusters)")
        print("Davies-Bouldin Index: Not applicable (invalid number of clusters)")
        print(f'Adjusted Rand Index (ARI): {adjusted_rand_score(true_labels, predicted_labels):.6f}')
        print(f'Adjusted Mutual Information (AMI): {adjusted_mutual_info_score(true_labels, predicted_labels):.6f}')
...
# plot_pca_2d(data_normalized, predicted_labels_dbscan, 'PCA of Protein Data with DBSCAN Clusters', 'pca_clusters_DBSCAN.png')
...
print(f'Time taken: {end_time - start_time} seconds')
# plot_pca_2d(data_normalized, predicted_labels_meanshift, 'PCA of Protein Data with Mean Shift Clusters', 'pca_clusters_MeanShift.png')
...
print(f'Time taken: {end_time - start_time} seconds')
# plot_pca_2d(data_normalized, predicted_labels_hdbscan, 'PCA of Protein Data with HDBSCAN Clusters', 'pca_clusters_HDBSCAN.png')
...
print(f'Time taken: {end_time - start_time} seconds')
# plot_pca_2d(data_normalized, predicted_labels_quickshift, 'PCA of Protein Data with QuickShift Clusters', 'pca_clusters_QuickShift.png')
