...
def build_CCgraph(X, k, cutoff, n_jobs):
    ...
    CCmat = kneighbors_graph(neighbors, k, mode='distance', include_self=False)
...
def get_density_dists_bb(X, k, components, knn_radius, n_jobs):
    ...
    for cc in comps:
        ...
        index.add(X[cc_idx, :])
        distances, neighbors = index.search(X[cc_idx, :], kcc)
        ...
        for indx_chunk in utils.chunks(search_idx, 100):
            ...
            if any(np.sum(GT_radius, axis=1) == 0):
                max_i = [i for i in range(
                    GT_radius.shape[0]) if np.sum(GT_radius[i, :]) == 0]
                if len(max_i) > 1:
...
def get_density_dists_bb_optimized(X, k, components, knn_radius, n_jobs=1):
    """
    Optimized Step 2: Compute density distances and big brothers using mutual k-nearest neighbor graph.
    """
    best_distance = np.full(X.shape[0], np.nan)
    big_brother = np.full(X.shape[0], np.nan)
    comps = np.unique(components[~np.isnan(components)]).astype(int)
    ...
    for cc in comps:
        ...
        cc_knn_radius = knn_radius[cc_idx]
        cc_best_distance = np.full(nc, np.nan)
        cc_big_brother = np.full(nc, np.nan)

        cc_radius_diff = cc_knn_radius[:,
                                       np.newaxis] - cc_knn_radius[neighbors]
        valid_rows, valid_cols = np.where(cc_radius_diff > 0)
        unique_rows, first_occurrence_idx = np.unique(
            valid_rows, return_index=True)
        unique_cols = valid_cols[first_occurrence_idx]

        cc_best_distance[unique_rows] = distances[unique_rows, unique_cols]
        cc_big_brother[unique_rows] = neighbors[unique_rows, unique_cols]

        search_idx = np.setdiff1d(np.arange(nc), unique_rows)

        for indx_chunk in np.array_split(search_idx, np.ceil(len(search_idx) / 100)):
            search_radius = cc_knn_radius[indx_chunk]
            GT_radius = cc_knn_radius < search_radius[:, np.newaxis]

            GT_distances = [(X[cc_idx[indx_chunk[i]]], X[cc_idx[GT_radius[i, :]]])
            ...
            if GT_radius.shape[0] > 50:
                try:
                    pool = mp.Pool(processes=n_jobs)
                    distances = pool.map(
                        utils.density_broad_search_star, GT_distances)
                    argmin_distance = [np.argmin(l) for l in distances]
                    pool.terminate()
                except Exception as e:
                    print("POOL ERROR: " + str(e))
                ...
                distances = list(
                    map(utils.density_broad_search_star, GT_distances))
                argmin_distance = [np.argmin(l) for l in distances]

            for i in range(GT_radius.shape[0]):
                cc_big_brother[indx_chunk[i]] = np.where(
                    GT_radius[i, :])[0][argmin_distance[i]]
                cc_best_distance[indx_chunk[i]
                                 ] = distances[i][argmin_distance[i]]

        big_brother[cc_idx] = [cc_idx[int(i)] for i in cc_big_brother]
        best_distance[cc_idx] = cc_best_distance
...
def get_y(CCmat, components, knn_radius, best_distance, big_brother, rho, alpha, d):
    ...
    for cc in comps:
        ...
        # Convert Big Brother into Component Big Brother
        index = np.argsort(cc_idx)
        sorted_x = cc_idx[index]
        sorted_index = np.searchsorted(sorted_x, big_brother[cc_idx])
        cc_big_brother = np.take(index, sorted_index, mode="clip")
...
class CPFcluster:
    ...
    def fit(self, X):
        ...
        start_time_step2 = time.time()
        best_distance, big_brother = get_density_dists_bb_optimized(
            X, self.k, self.components, knn_radius, self.n_jobs)
        step2_time = time.time() - start_time_step2
        print(f"Step 2: Get Density Dists BB took {step2_time:.4f} seconds")
...
if __name__ == '__main__':
    ...
    np.random.seed(42)
    # Assuming 7 clusters for synthetic labels
    true_labels = np.random.randint(0, 7, size=data_normalized.shape[0])
