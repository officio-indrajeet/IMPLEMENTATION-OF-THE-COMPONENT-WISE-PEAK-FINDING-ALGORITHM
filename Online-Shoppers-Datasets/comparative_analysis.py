...
data_numerical = scaler.fit_transform(data[numerical_features])

# Preprocessing for categorical data (One-Hot Encoding)
data_categorical = pd.get_dummies(data[categorical_features], drop_first=True)

# Combine numerical and categorical data
data_combined = np.hstack((data_numerical, data_categorical))

# Add synthetic labels for ARI and AMI calculations
np.random.seed(42)
true_labels = np.random.randint(0, 5, size=data_combined.shape[0])
...
def print_clustering_metrics(true_labels, predicted_labels):
    ...
    silhouette_avg = silhouette_score(data_combined, predicted_labels)
    davies_bouldin = davies_bouldin_score(data_combined, predicted_labels)
...
predicted_labels_dbscan = dbscan.fit_predict(data_combined)
end_time = time.time()
...
# plot_pca_2d(data_combined, predicted_labels_dbscan, 'PCA of Online Shoppers Data with DBSCAN Clusters', 'pca_clusters_DBSCAN.png')

# Mean Shift
start_time = time.time()
mean_shift = MeanShift()
predicted_labels_meanshift = mean_shift.fit_predict(data_combined)
end_time = time.time()
...
print(f'Time taken: {end_time - start_time} seconds')
# plot_pca_2d(data_combined, predicted_labels_meanshift, 'PCA of Online Shoppers Data with Mean Shift Clusters', 'pca_clusters_MeanShift.png')

# HDBSCAN
start_time = time.time()
hdbscan_clustering = hdbscan.HDBSCAN(min_cluster_size=15)
predicted_labels_hdbscan = hdbscan_clustering.fit_predict(data_combined)
end_time = time.time()
...
print(f'Time taken: {end_time - start_time} seconds')
# plot_pca_2d(data_combined, predicted_labels_hdbscan, 'PCA of Online Shoppers Data with HDBSCAN Clusters', 'pca_clusters_HDBSCAN.png')
...
print(f'Time taken: {end_time - start_time} seconds')
# plot_pca_2d(data_combined, predicted_labels_quickshift, 'PCA of Online Shoppers Data with QuickShift Clusters', 'pca_clusters_QuickShift.png')
